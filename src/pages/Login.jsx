...
const Login = () => {
  ...
  useEffect(() => {
    window.scrollTo(0, 0);
    document.title = "Tiệm Giày DOM | Đăng nhập";
  }, []);
  ...
  const handleForgetPassword = () => {
    if (!username) {
      setErrorMess("Vui lòng nhập trường username");
      setIsShowError(true);
      return;
    }
    apiService
      ...
      .catch((err) => {
        setErrorMess(err?.response?.data?.message || "Có lỗi xảy ra");
        setIsShowError(true);
      });
  };
  const handleLogin = () => {
      ...
      <div className="flex min-h-[80vh] flex-col items-center justify-center">
        <div className="m-4 grid w-full max-w-6xl items-center gap-4 rounded-md p-4 shadow-[0_2px_10px_-3px_rgba(6,81,237,0.3)] max-md:max-w-lg max-md:gap-8 md:grid-cols-2">
              ...
              <div className="mt-6 flex flex-wrap items-center justify-between gap-4">
                <div>
                  <span
                    ...
                    className="text-sm font-semibold text-[#FF3D00] hover:underline"
                  >
                    Quên mật khẩu?
                  </span>
                </div>
          ...
          <div className="bg-[#000842] md:h-full">
            <img
              src="https://file.hstatic.net/1000230642/file/kham_pha_-_600_x_890.jpg"
